    ...
    config_data = {
        "mcpServers": {
            "test-mssql": {
                ...
                "command": "node",
                "args": ["/path/to/mcp.js"],
                "env": {"SERVER_NAME": "localhost", "DATABASE_NAME": "TestDB"},
                "url": None,
                "headers": {},
                "enabled": True,
                "readonly": False,
...
class TestMCPEnvironmentVariables:
    ...
    def test_env_var_expansion(self, tmp_path):
        ...
        config_data = {
            "mcpServers": {
                "env-test": {
                    ...
                    "command": "node",
                    "args": ["${MCP_PATH}"],
                    "env": {"HOST": "${TEST_HOST}", "PORT": "${TEST_PORT:-8080}"},
                    "enabled": True,
        ...
        with patch.dict(
            "os.environ", {"MCP_PATH": "/actual/path/to/mcp.js", "TEST_HOST": "testhost"}
        ):
...
class TestMCPErrorHandling:
    ...
    def test_server_connection_failure_logged(self, mcp_manager):
        ...
        with (
            patch("src.mcp.client.MCPServerStdio", side_effect=Exception("Connection failed")),
            patch("src.mcp.client.logger") as mock_logger,
        ):
            mcp_manager.get_active_toolsets()
